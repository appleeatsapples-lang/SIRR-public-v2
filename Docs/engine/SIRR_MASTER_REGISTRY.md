...
- `Engine/web_backend/sanitize.py` (`hash_oid` truncation length) — operational log correlation IDs are a 12-char SHA-256 prefix (~48 bits of preimage resistance). Acceptable for the current single-tenant log surface and threat model, but flagged as a strength upgrade (HMAC-keyed variant) to revisit when the customer base grows past ~10⁵ active orders or the log surface becomes multi-tenant. See lower deferred section for canonical entry.
...
- **`Engine/web_backend/server.py:869`** (deletion gap) — `POST /api/delete` calls `update_order(status="deleted", profile=None, email_hash=None, reading_url=None, error=None)`. This sets four fields to None and rewrites the row in place. It does NOT touch `name_latin`, `name_arabic`, `dob`, `birth_time`, or `birth_location` — those PII fields remain in the order row after a successful deletion. P2G must extend the deletion call to null all PII fields (or unlink the row entirely) so deletion actually removes the customer's identity from disk.
